    ...
    async execute(interaction) {
        ...
        if (sub === "create") {
            ...
            const footerInput = new TextInputBuilder()
                .setCustomId("footer").setLabel("Footer text (optional)").setStyle(TextInputStyle.Short).setRequired(false);
            const imageInput = new TextInputBuilder()
                .setCustomId("image").setLabel("Image URL (bottom image, optional)").setStyle(TextInputStyle.Short).setRequired(false);
        ...
        if (sub === "load") {
            ...
            try {
                ...
                // Store messageId for reaction roles
                if (data.reactionRoles && data.reactionRoles.length > 0) {
                    if (!embeds[interaction.guildId]) embeds[interaction.guildId] = {};
                    embeds[interaction.guildId][name].messageId = msg.id;
                    await writeJSON("embeds.json", embeds);
                    ...
                    for (const rr of data.reactionRoles) {
                        try {
                            ...
                            console.error("Failed to react with " + rr.emoji + ":", e.message);
                        }
                    }
                }
        ...
        if (sub === "edit") {
            ...
            const titleInput = new TextInputBuilder()
                .setCustomId("title").setLabel("Title (optional)").setStyle(TextInputStyle.Short)
                .setValue(data.title || "").setRequired(false);
            const descInput = new TextInputBuilder()
            ...
            const footerInput = new TextInputBuilder()
                .setCustomId("footer").setLabel("Footer (optional)").setStyle(TextInputStyle.Short)
                .setValue(data.footer || "").setRequired(false);
            const imageInput = new TextInputBuilder()
                .setCustomId("image").setLabel("Image URL (optional)").setStyle(TextInputStyle.Short)
